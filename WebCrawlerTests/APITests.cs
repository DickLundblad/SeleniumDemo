...
namespace WebCrawler
{
    ...
    public class APITests
    {
        ...
        [TestCase("https://www.linkedin.com/jobs/collections/it-services-and-it-consulting", "//div[@data-job-id]", "linkedin_com_it-services-and-it-consulting", "")]
        [TestCase("https://www.linkedin.com/jobs/search/?keywords=software", "//div[@data-job-id]", "linkedin_com_software", "")]
        public async Task CrawlAsyncStartPageForJoblinks_ParseJobLinks_WriteToFile(string url, string selectorXPathForJobEntry, string fileName, string addDomainToJobPaths = "", int delayUserInteraction = 0, bool removeParams = true, string folderPathToResultFiles = "")
        {
             var progress = new Progress<CrawlProgressReport>();
            await _api.CrawlAsynchStartPageForJoblinks_ParseJobLinks_WriteToFile(url, selectorXPathForJobEntry, fileName, progress, addDomainToJobPaths, delayUserInteraction, removeParams);
        }
